...
function renderMatrixSemulaMenjadi(rows, semulaTipe, menjadiTipe) {
    ...
    // Update Header Text
    // Cari bagian update Header Text
if (headerTitle) headerTitle.textContent = `Perbandingan Anggaran: ${semulaTipe.replace('Perubahan', 'Revisi')} vs ${menjadiTipe.replace('Perubahan', 'Revisi')}`;
if (headerSemula) headerSemula.textContent = `Total ${semulaTipe.replace('Perubahan', 'Revisi')}`;
if (headerMenjadi) headerMenjadi.textContent = `Total ${menjadiTipe.replace('Perubahan', 'Revisi')}`;
...
function renderUserGuide() {
  ...
  function updatePerubahanUI(settings) {
    ...
    // VARIABLE INTERNAL (JANGAN UBAH)
    const tahapStrInternal = `Perubahan ${tahapAktif}`; 
    // VARIABLE TAMPILAN (UBAH KE REVISI)
    const tahapStrDisplay = `Revisi ${tahapAktif}`;
    ...
    if (isTahapPerubahanOpen) {
      ...
      // GUNAKAN tahapStrDisplay
      if (linkAjuanPerubahan) linkAjuanPerubahan.innerHTML = `<i class="bi bi-pencil-square"></i> Buat Ajuan ${tahapStrDisplay}`;
      if (linkDaftarPerubahan) linkDaftarPerubahan.innerHTML = `<i class="bi bi-list-check"></i> Daftar Ajuan ${tahapStrDisplay}`;
      if (linkRpdPerubahan) linkRpdPerubahan.innerHTML = `<i class="bi bi-calendar2-event"></i> RPD ${tahapStrDisplay}`;
      if (linkRealisasiPerubahan) linkRealisasiPerubahan.innerHTML = `<i class="bi bi-graph-up-arrow"></i> Realisasi ${tahapStrDisplay}`;
    }
    ...
    // GUNAKAN tahapStrDisplay
    if(daftarPerubahanTitle) daftarPerubahanTitle.innerHTML = `<i class="bi bi-list-check"></i> Daftar Ajuan ${tahapStrDisplay}`;
    if(rpdPerubahanTitle) rpdPerubahanTitle.innerHTML = `<i class="bi bi-calendar2-event"></i> RPD ${tahapStrDisplay}`;
    if(realisasiPerubahanTitle) realisasiPerubahanTitle.innerHTML = `<i class="bi bi-graph-up-arrow"></i> Realisasi ${tahapStrDisplay}`;
    ...
    if (copyBtn) {
        if (isTahapPerubahanOpen && STATE.role === 'prodi') { 
             ...
             // Logic Tampilan (Replace Perubahan -> Revisi)
             const sourceDisplay = sourceStr.replace('Perubahan', 'Revisi');
             const destDisplay = destinationStr.replace('Perubahan', 'Revisi');

             copyBtn.innerHTML = `<i class="bi bi-files"></i> Pindahkan Ajuan Diterima (${sourceDisplay} -> ${destDisplay})`;
             copyBtn.title = `Salin semua ajuan dari tahap ${sourceDisplay} yang diterima ke daftar ${destDisplay}`;
        } else {
             copyBtn.style.display = 'none';
        }
    }
...
safeAddClickListener('btn-refresh-matrix', loadMatrixSemulaMenjadi);
  ...
  document.getElementById('link-ajuan-perubahan').addEventListener('click', () => {
    const tahapAktif = STATE.globalSettings.Tahap_Perubahan_Aktif || 1;
    STATE.currentAjuanType = `Perubahan ${tahapAktif}`; // Internal tetap Perubahan
    
    const titleEl = document.getElementById('ajuan-form-title');
    // Tampilan diubah jadi Revisi
    if(titleEl) titleEl.innerHTML = `<i class="bi bi-pencil-square"></i> Formulir Ajuan Revisi ${tahapAktif}`;
...
async function loadDashboardData(forceRefresh = false) { 
 ...
 // --- FIX: renderDashboardSummary (Scope Fixed) ---
function renderDashboardSummary(data, containerPrefix = 'dashboard-', chartPrefix = 'chart') { 
    
    // 1. DEFINISIKAN VARIABEL INI DI PALING ATAS (Agar tidak kena ReferenceError)
    const isPerubahanOpen = STATE.globalSettings.Status_Ajuan_Perubahan === 'Dibuka';
    const tahapAktif = STATE.globalSettings.Tahap_Perubahan_Aktif || 1;
